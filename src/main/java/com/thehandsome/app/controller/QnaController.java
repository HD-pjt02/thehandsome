...
public class QnaController {
   ...
   public String qnaInsertGet(HttpSession session) {
      ...
      return "/qna/mtmInqrReg";
   }

   // qna입력 페이지
   @RequestMapping(value = "/mtmInqrReg", method = RequestMethod.POST)
   public String qnaPOST(MultipartHttpServletRequest request, QnaDTO qna, MultipartFile[] uploadFile)
         throws Exception {
...
//      }
   ...
   @RequestMapping(value = "customerCenterMain", method = RequestMethod.GET)
   public String qnaSelectGet(PagingDTO vo, Model model, HttpSession session,
   ...
   @ResponseBody
   public String qnaDelete(@RequestParam("qid") String qid) {
      log.info("qna 삭제 진입");
      qnaservice.qnadelect(Integer.parseInt(qid));
